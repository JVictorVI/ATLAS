...
| `AtlasEngineDownloadService`             | Implementado com detecção automática CPU/CUDA/Vulkan, busca e atualização por release, registro central das três engines, troca segura, validação e DLLs CUDA complementares |
...
| `AtlasSystemPromptPolicyService`         | Implementado com prompts especializados, análise arquitetural em oito tópicos, sugestões breves e acessíveis de refatoração e política JSON para análise rápida  |
...
| Remoção em massa do RAG        | Projetos indexados só podiam ser excluídos individualmente.                                                 | A tela oferece **Remover todos**, com confirmação e limpeza sequencial somente dos índices; materiais complementares são preservados.                                             |
...
| Restauração de padrões        | A ação de UI não estava documentada.                                                                     | `restaurarConfiguracoesAtlas` restaura defaults gerais sem alterar a engine local nem apagar provedores, chaves, modelos, índices RAG ou histórico.                          |
...
| Atualização da engine         | O download escolhia um release recente, mas instalações existentes não tinham versão registrada nem comparação posterior. | O registro central mantém CPU/CUDA/Vulkan e a UI mostra as três versões; a busca compara somente a engine selecionada e exige "Atualizar agora" antes de substituir os binários. |
